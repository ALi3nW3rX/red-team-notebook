...
{% embed url="https://github.com/Kevin-Robertson/Inveigh#parameter-help" %}
...
After typing <mark style="color:green;">`HELP`</mark> and hitting enter, we are presented with several options:
...
We can quickly view unique captured hashes by typing <mark style="color:green;">`GET NTLMV2UNIQUE`</mark>.
...
We can type in <mark style="color:green;">`GET NTLMV2USERNAMES`</mark> and see which usernames we have collected. This is helpful if we want a listing of users to perform additional enumeration against and see which are worth attempting to crack offline using Hashcat.
